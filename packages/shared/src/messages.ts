...
export interface ScopeManifestCategory {
  canonicalCategoryKey: string;
  categoryName: string;
  categoryOrder: number;
}

export interface CreateScopeManifestInput {
  storeId: string;
  includeCoupons: boolean;
  categories: ScopeManifestCategory[];
  sourceArtifactId?: string;
}

export interface ScopeManifestRecord extends CreateScopeManifestInput {
  scopeManifestId: string;
  scopeVersion: string;
  manifestHash: string;
  frozenAt: string;
...
export interface RegisterPriceQualityInput {
  ...
  dynamicLabelPresent?: number;
  duplicateSpuCount?: number;
  /** Worker hint only. Master recalculates and owns the persisted decision. */
  completenessStatus?: "pass" | "warn" | "fail";
  metadata?: Record<string, unknown>;
  leaseOwner?: string;
  leaseGeneration?: number;
