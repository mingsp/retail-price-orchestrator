...
66 与 201 已在独立目录完成 `v0.2.5` 候选验证，但未切换现有生产服务。当前仍不能把项目表述为“66/201 已正式升级”或“新人已独立接管”；这两项属于 L3～L4 现场门禁，必须分别完成备份、Canary、稳定窗口和签字验收。
...
| 候选镜像身份 | 通过 | 固定版本、40 位 commit、结构版本、离线包 SHA-256、源 config digest、201 加载后 image ID 和旧镜像回滚目标均已记录；未启动候选镜像 |
...
- 201 候选位于 `D:\SpanAI\retail-radar-master\sources\v0.2.5`；66 候选位于 `C:\ProgramData\RetailRadar\Standalone\sources\v0.2.5`。
- 两端均记录 `candidate_verified`、`activation=not_switched`、Node 22.14.0、pnpm 11.21.0；原源码目录和运行容器保持不变。
- 201 已生成并验签 `v0.2.5-signed` 三平台 Worker 包，并离线加载但未启动候选 Master/Dashboard 镜像；旧运行镜像 ID 已作为回滚目标固化。
- 两端尚未切换到 0.2.5。
