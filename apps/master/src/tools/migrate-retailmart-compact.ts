...
const tables = [
  {
    ...
    stage: "fact_store_spu_price_snapshot_compact_20260714",
    backup: "fact_store_spu_price_snapshot_precompact_20260714",
    removed: compactSpuRemovedColumns,
  ...
  },
  {
    ...
    stage: "fact_store_sku_price_snapshot_compact_20260714",
    backup: "fact_store_sku_price_snapshot_precompact_20260714",
    removed: compactSkuRemovedColumns,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (states.every((state) => state === "compact")) {
      process.stdout.write("两张价格快照表已经是精简结构，无需重复迁移。\n");
      return;
    }
    ...
    for (const table of tables) {
      ...
      const removedColumns = new Set<string>(table.removed);
      ...
      if (sourceCount !== stageCount) {
        throw new Error(`migration_verify_failed:${table.label}_count_${sourceCount}_${stageCount}`);
      }
      const mismatchCount = await copiedRowMismatchCount(connection, table.current, table.stage, retainedColumns);
      if (mismatchCount !== 0) throw new Error(`migration_verify_failed:${table.label}_row_mismatch_${mismatchCount}`);
      const stageColumns = await tableColumns(connection, table.stage);
      if (classifyFactSchema(stageColumns, table.removed) !== "compact") {
        throw new Error(`migration_verify_failed:${table.label}_stage_not_compact`);
      }
    }

    for (const table of tables) {
      const latestSourceCount = await tableCount(connection, table.current);
      if (latestSourceCount !== expectedCounts.get(table.current)) {
    ...
    for (const table of tables) {
      ...
      process.stdout.write(`${table.label} 精简表 ${currentCount} 行，迁移前备份 ${backupCount} 行。\n`);
    }
  } finally {
...
async function copiedRowMismatchCount(
  ...
  const comparisons = columns
    ...
    .map((column) => `NOT (t.${quoteIdentifier(column)} <=> s.${quoteIdentifier(column)})`)
    .join(" OR ");
  const [rows] = await connection.query<any[]>(
    `SELECT COUNT(*) AS count FROM ${quoteIdentifier(source)} s LEFT JOIN ${quoteIdentifier(target)} t ON t.id=s.id WHERE t.id IS NULL OR ${comparisons}`
  );
  return Number(rows[0]?.count || 0);
}
