...
test("production readiness blocks worker dead letters and unfrozen active runs", () => {
  const worker = makeWorker();
  const report = buildProductionReadinessReport({
    workers: [{
      ...worker,
      execution: {
        capture: { name: "capture", concurrency: 1, maxQueueSize: 1, active: 0, waiting: 0, rejected: 0, completed: 0, failed: 0, queueWaitMsP95: 0 },
        productPipeline: { name: "pipeline", concurrency: 1, maxQueueSize: 1, active: 0, waiting: 0, rejected: 0, completed: 0, failed: 0, queueWaitMsP95: 0 },
        pressure: { level: "L0", memoryUsedRatio: 0.4, eventLoopDelayMsP95: 1, acceptingNewCapture: true },
        outbox: { pending: 0, inflight: 0, retryableFailure: 0, deadLetter: 2 },
    ...
    runs: [{
      runId: "run-1",
      runBusinessKey: "a".repeat(64),
      storeId: "store-1",
      runLabel: "2026-W32",
      scheduleWindow: "2026-W32",
      scopeVersion: "scope:unfrozen-v1",
      status: "planned",
  ...
  assert.equal(report.status, "blocked");
  assert.ok(report.issues.some((issue) => issue.id === "worker:outbox-dead-letter:mm-worker"));
  assert.ok(report.issues.some((issue) => issue.id === "run:scope-unfrozen:run-1"));
});

test("production readiness requires runtime accounts in the production account pool", () => {
  const report = buildProductionReadinessReport({
    workers: [makeWorker()],
    stores: [],
    runs: [],
  ...
  assert.ok(report.issues.some((issue) => issue.id === "account:pool-missing:account-a"));
});
