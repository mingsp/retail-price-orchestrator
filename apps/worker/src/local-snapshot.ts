...
import { getMutationSpoolSnapshot } from "./master-api.js";
...
export async function buildHeartbeat(config: WorkerConfig): Promise<WorkerHeartbeatPayload> {
  ...
  const execution = {
    ...getExecutionRuntime().snapshot(),
    outbox: await getMutationSpoolSnapshot(config)
  };
...
export function buildExecutionSummary(execution: WorkerHeartbeatPayload["execution"]): string {
  ...
  if (execution.outbox?.deadLetter) parts.push(`待人工复核 ${execution.outbox.deadLetter} 项`);
  else if (execution.outbox?.retryableFailure) parts.push(`待自动重试 ${execution.outbox.retryableFailure} 项`);
