...
export async function getMutationSpoolSnapshot(config: WorkerConfig) {
  const stats = await getMutationSpool(config).stats();
  return {
    pending: stats.pending,
    inflight: stats.inflight,
    retryableFailure: stats.retryable_failure,
...
export function startMutationSpoolReplay(config: WorkerConfig): () => void {
  ...
  const replay = async () => {
    ...
    try {
      ...
      for (const item of await spool.claim(25, `${config.worker.workerId}:${process.pid}`)) {
        try {
          ...
          if (response.ok) {
            await spool.acknowledge(item.idempotencyKey);
          } else if (response.status >= 500 || response.status === 429) {
            await spool.fail(item.idempotencyKey, `HTTP ${response.status}`);
          } else {
            await spool.fail(item.idempotencyKey, `non_retryable_http_${response.status}`);
          }
        } catch (error) {
          await spool.fail(item.idempotencyKey, error instanceof Error ? error.message : String(error));
        }
      }
    } finally {
      running = false;
